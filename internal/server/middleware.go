...
func authorization(ctx *gin.Context) {
	...
	if authHeader == "" {
		ctx.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "Auth heeader missing"})
		return
	}
	headerParts := strings.Split(authHeader, " ")
	if len(headerParts) != 2 {
		ctx.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "Auth heeader miss formed"})
		return
	}
...
func currentUser(ctx *gin.Context) (*store.User, error) {
	...
	if !exists {
		err = errors.New("current user not set")
