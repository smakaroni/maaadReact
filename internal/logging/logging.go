...
var logFilePath = filepath.Join(logDir, logName)
...
func createLogDir() {
	if err := os.Mkdir(logDir, 0744); err != nil && !os.IsExist(err) {
...
func backupLastLog() {
	...
	bkpLogPath := filepath.Join(logDir, bkpLogName)
...
func currentDir() string {
	...
	if err != nil {
		log.Panic().Err(err).Msg("Cannot get current directory")
	}
